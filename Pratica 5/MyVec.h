...
//A complexidade do eraseMatchingElements() é O(n²) no pior caso
template<class T>
int MyVec<T>::eraseMatchingElements(const T &elem){    //argumento constante e passado por referencia
	int cont = 0;
	...
	for(int i = 0; i < dataSize; i++){
		if(data[i] == elem){
			...
			for(int j = i; j < dataSize-1; j++)
				std::swap(data[j], data[j+1]);     //coloca o elemento igual na ultima posicao
			i--;                                   //subtrai 1 no i para verificar o elemento com que foi trocado
			pop_back();                            //Tira a ultima casa do vetor
		}
	}
...
//Algoritmo eraseMatchingElements(); utilizando alocacao de um novo vetor
/*
template<class T>
...
void  MyVec<T>::sortedInsert(const T &elem){
	if(dataSize == 0 || elem <= data[0]){
      insert(elem, 0);
      return;
	}
  
	for(int i = 0; i < dataSize-1; i++){
		if(elem >= data[i] && elem <= data[i+1]){
    		insert(elem, i+1);
		return;
		}
	}
