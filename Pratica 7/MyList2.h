...
public:
	...
	int size() const; // na STL List, a funcao size() calcula o tamanho da lista dinamicamente (exercicio: qual a ordem de complexidade?)
	int eraseMatchingElements(T &elem);
	void reverse();
	void reverse(Node<T>*rev, Node<T>* aux);
	bool compare(const iterator it1, const iterator it2) const;
	void comp(const iterator it1, const iterator it2, bool &teste) const;
...
template<class T>
  void MyList2<T>::reverse(){
    ...
    Node<T> *ptr1;
    Node<T> *ptr = dataFirst;
    reverse(dataFirst, ptr1);
    dataFirst = dataLast;
    dataLast = ptr;
  }

  template<class T>
  void MyList2<T>::reverse(Node<T>*rev, Node<T>* ptr1){
    if(size()==0){
      	return;
...
template<class T>
void MyList2<T>::comp(const iterator it1, const iterator it2, bool &aux) const{
	if(it2->prev){    //se it2->prev apontar pra algo
		if(it2->prev == it1){              
			aux = true;
			return;
		}
		comp(it1, it2->prev, aux);
	}
}
